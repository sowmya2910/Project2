...
CB_t TXBuff;
uint8_t ch;
#define divisor ((CORE_CLOCK) / (baud_rate * 16));
...
void UART_configure(){
    ...
    UART0->C2 &= ~(UARTLP_C2_RE_MASK | UARTLP_C2_TE_MASK); /* Disable the transmitter and receiver */
    ...
    UART0->S2 = 0; /* Initialize LSB */
    /* Clear C1, C2, C3,C5 to disable UART transmitter and receiver */
    UART0->C2 = 0;
    UART0->C1 = 0;
    UART0->C3 = 0;
    UART0->C5 = 0;
    UART0->C4 = UARTLP_C4_OSR(16-1); /* Set oversampling rate to 16 */
    ...
    UART0->C2 |= UARTLP_C2_RE_MASK | UARTLP_C2_TE_MASK | UARTLP_C2_RIE_MASK;
